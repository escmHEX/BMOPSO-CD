...
| `MOPSO` | Inertia memory | \(H_{i,d}^t\) stores `movement`, `target_position`, `accepted_component`, and `iteration` only after an accepted cognitive/social move. | `Solution.last_guided_move`; `mopso._guided_move_memory`; persisted in solution JSON/checkpoints. | OK |
| `MOPSO` | Inertia selection weight | \(a_{in}=\omega(t)\lvert v_{i,d}^{t}\rvert\) only if \(H_{i,d}^t\) is cognitive/social and `iteration=t-1`; otherwise \(a_{in}=0\). | `mopso._valid_inertia_memory`; `s_in_weight`; used only for move-source weighting. | OK |
| `MOPSO` | Inertia target | If inertia is selected, reuse \(H_{i,d}^t.movement\) and \(H_{i,d}^t.target\_position[d]\), then consume \(H_{i,d}^t\). Inertia does not create a new inertia memory. | `mopso._movement_context`; `mopso._update_particle`; same logic in async updates. | OK |
...
| `MOPSO` | Inertia endpoints | \(\omega_{max},\omega_{min}\) | `0.9`, `0.4` | `mopso.omega_max`, `mopso.omega_min` | Linear decreasing inertia; reuse is gated by one-generation memory. | OK |
...
| `estrategia_mopso_cd_semantica_v17_anclas_probabilisticas_inercia_memoria_consumible.html` | Discrete MOPSO-CD with active/frozen components, schedules in `[0,1]`, `Dmax`, `Kcand`, no retries, evaluate modified generation in batch, and consumable inertia memory toward the previous accepted target. | `src/binary_mopso_cd/mopso.py`, `src/binary_mopso_cd/entities.py`, `src/binary_mopso_cd/component_memory.py` | `mopso.*`, `experiment.frozen_components` | `tests/unit/test_mopso_core.py`, `tests/unit/test_runtime_schedules.py`, `tests/unit/test_frozen_components.py`, `tests/integration/test_reduced_runs.py` |
