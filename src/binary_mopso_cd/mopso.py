...
class BinaryMOPSOCDEngine:
    ...
    def _update_population(self, population: list[Solution], pbest: list[Solution], generation: int) -> list[Solution]:
        if not self.parallelism.enabled or self.parallelism.particle_update_max_concurrent <= 1:
            ...
            for index, particle in enumerate(population):
                ...
                next_population.append(self._update_particle(particle, pbest[index], leader, generation))
            return next_population
        leaders = [self.archive.select_leader(self.mopso.leader_tournament_size) for _particle in population]
        jobs = [
            ParticleUpdateJob(index, particle.clone(keep_id=True), pbest[index].clone(), leaders[index].clone(), generation)
            for index, particle in enumerate(population)
        ]
        results = run_async(self._update_population_parallel(jobs))
        errors = [
            {"phase": "optimization", "generation": generation, "index": result.index, "error": result.error}
            for result in results
            if result.error is not None
        ]
        if errors:
            self._write_particle_update_errors(errors)
        rejections = [row for result in results for row in result.optimization_rejections]
        if rejections:
            self._write_optimization_rejections(rejections)
        return [result.solution for result in results]

    async def _update_population_parallel(self, jobs: list[ParticleUpdateJob]) -> list[ParticleUpdateResult]:
        async def worker(job: ParticleUpdateJob) -> ParticleUpdateResult:
            rng = Random(particle_update_seed(self.config.seed, job.generation, job.index))
            try:
                solution, rejections = await self._update_particle_async(
                    job.particle,
                    job.pbest,
                    job.leader,
                    job.generation,
                ...
                restored = job.particle.clone(keep_id=True)
                restored.changed = False
                return ParticleUpdateResult(job.index, restored, str(exc))

        return await run_limited(jobs, self.parallelism.particle_update_max_concurrent, worker)
    ...
    def _update_particle(
        ...
        for component, mode, _weight, s_in_weight, s_cog, s_soc in candidates:
            if mode == "guided":
                guided_mode = self._guided_mode(component, s_in_weight, s_cog, s_soc, particle, rng)
        ...
        if updated.changed:
            ...
            used_central_anchors, anchor_probability = self._central_anchor_usage(generation, rng)
    ...
    def _candidate_for_mode(
        ...
        if effective_mode == "turbulence":
            return self._turbulence_candidate(component, current, rng)
    ...
    async def _candidate_for_mode_async(
        ...
        if effective_mode is None:
            return None
        if effective_mode == "turbulence":
            return await self._turbulence_candidate_async(component, current, rng)
        target = pbest.vector.components[component] if effective_mode == "cognitive" else leader.vector.components[component]
        route = RouteTask(
            uuid4().hex,
            "optimization",
        ...
        raw_candidates = list(await self._execute_route_async(route))
        return self._select_guided_candidate(component, current, target, raw_candidates)
    ...
    async def _turbulence_candidate_async(self, component: str, current: str, rng: Random) -> str | None:
        spec = component_spec(component)
        units = self.executor.turbulence_provider.modifiable_units(current, spec.preferred_turbulence_pos)
        if not units:
            return None
    ...
    async def _render_prompt_async(self, vector: SemanticVector) -> str:
        ...
        return str(await self._execute_route_async(route))
    ...
    async def _generate_text_async(self, prompt: str, user_prompt_override: str | None = None) -> str:
        ...
        return str(await self._execute_route_async(route)).strip()

    async def _execute_route_async(self, route: RouteTask) -> Any:
