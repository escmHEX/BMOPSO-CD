...
class MOPSOSettings:
    ...
    def from_config(cls, config: RuntimeConfig) -> "MOPSOSettings":
        return cls(
            ...
            kcand=int(config.get("mopso.kcand", 5)),
            ...
            alpha=float(config.get("mopso.alpha", 0.5)),
            p_tur_max=float(config.get("mopso.p_tur_max", 0.05)),
            p_tur_min=float(config.get("mopso.p_tur_min", 0.01)),
